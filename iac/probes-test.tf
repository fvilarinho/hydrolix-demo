...
resource "local_file" "probesTest" {
  ...
  filename = "${local.probesSourceScriptsDirectory}/${var.settings.probes.prefix}-${each.value.region}-${each.key}.sh"
  content  = <<EOT
#!/bin/bash

# Required environment.
export LOGS_DIR="${local.probesDestinationLogsDirectory}"
